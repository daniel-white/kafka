...
impl Uuid {
    ...
    ///
    /// Java uses `java.util.UUID.randomUUID()`. This Rust implementation uses a
    /// splitmix64-based PRNG seeded from the system clock. Note: in Phase B
    /// (native), this should use the `rand` crate via the `uuid` wrapper.
    ///
    /// MIGRATION_SOURCE: clients/src/main/java/org/apache/kafka/common/Uuid.java
    pub fn random_uuid() -> Self {
        let mut buf = [0u8; 16];
        get_random_bytes(&mut buf);
        // Set version (4) and variant (2) bits to match java.util.UUID
        buf[6] = (buf[6] & 0x0f) | 0x40;
        buf[8] = (buf[8] & 0x3f) | 0x80;
        let most = i64::from_be_bytes(buf[0..8].try_into().unwrap());
        let least = i64::from_be_bytes(buf[8..16].try_into().unwrap());
        let uuid = Uuid::new(most, least);
        // Avoid reserved UUIDs, matching Java behavior
        if uuid == Uuid::ZERO_UUID || uuid == Uuid::ONE_UUID {
